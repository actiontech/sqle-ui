...
import { mockUseInstance, mockUseRole, mockUseUsername } from "../../../../testUtils/mockRequest";
...
describe('User/RoleList/RoleLIstFilterForm', () => {
  ...
  test('should match snapshot', () => {
    const { container } = render(<RoleListFilterForm updateRoleListFilter={jest.fn()} />);
  ...
  test('should call update filter info when user click reset button', async () => {
    ...
    render(<RoleListFilterForm updateRoleListFilter={updateRoleListFilterMock} />);
    expect(instanceSpy).toBeCalledTimes(1);
    expect(usernameSpy).toBeCalledTimes(1);
    expect(roleSpy).toBeCalledTimes(1);
    await waitFor(() => {
